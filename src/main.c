...
void SSD1306_init() {
    ...
    uint8_t cmds[] = {
        ...
        0x00,                    // no offset
        SSD1306_SET_COM_PIN_CFG, // set COM (common) pins hardware configuration. Board specific
                                 // magic number. 0x02 Works for 128x32, 0x12 Possibly works for
        ...
        SSD1306_SET_SCROLL | 0x00, // deactivate horizontal scrolling if set. This is necessary as
...
static void SetPixel(uint8_t *buf, int x, int y, bool on) {
    ...
    // Each row is 128 long by 8 pixels high, each byte vertically arranged, so byte 0 is x=0,
    // y=0->7, byte 1 is x = 1, y=0->7 etc

    // This code could be optimised, but is like this for clarity. The compiler
    // should do a half decent job optimising it anyway.

    const int BytesPerRow =
        SSD1306_WIDTH; // x pixels, 1bpp, but each row is 8 pixel high, so (x / 8) * 8
...
#warning i2c / SSD1306_i2d example requires a board with I2C pins
    ...
    struct render_area area = {start_page : 0, end_page : (IMG_HEIGHT / SSD1306_PAGE_HEIGHT) - 1};
...
restart:
    ...
    // char *text[] = {"A long time ago", "  on an OLED ", "   display",     " far far away",
                    // "Lived a small",   "red raspberry", "by the name of", "    PICO"};
